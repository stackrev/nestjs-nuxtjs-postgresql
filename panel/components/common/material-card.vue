<template>
  <v-card :style="styles" v-bind="$attrs" v-on="$listeners">
    <HelperOffset v-if="hasOffset" :inline="inline" :full-width="fullWidth" :offset="offset">
      <v-card
        v-if="!$slots.offset"
        :color="color"
        :elevation="elevation"
        class="text-center v-card--material__header d-flex align-center"
        dark
        :min-height="height"
      >
        <slot v-if="!title && !text" name="header" />
        <!-- <div v-else class="px-5 py-1 header-ti"> -->
        <v-row v-else class="px-5 py-1 header-ti">
          <h4 class="IRANSans mb-2" v-text="title" />
          <v-spacer></v-spacer>
          <p class="category font-weight-thin mb-0" v-text="text" />
        </v-row>
        <!-- </div> -->
      </v-card>

      <slot v-else name="offset" />
    </HelperOffset>

    <v-card-text>
      <slot />
    </v-card-text>

    <v-divider v-if="$slots.actions" class="mx-3" />

    <v-card-actions v-if="$slots.actions">
      <slot name="actions" />
    </v-card-actions>
  </v-card>
</template>

<script>
import HelperOffset from '~/components/common/helper-offset.vue'
export default {
  components: {
    HelperOffset
  },
  inheritAttrs: false,
  props: {
    color: {
      type: String,
      default: 'primary'
    },
    elevation: {
      type: [Number, String],
      default: 10
    },
    inline: {
      type: Boolean,
      default: false
    },
    fullWidth: {
      type: Boolean,
      default: false
    },
    offset: {
      type: [Number, String],
      default: 24
    },
    height: {
      type: [Number, String],
      default: 80
    },
    title: {
      type: String,
      default: undefined
    },
    text: {
      type: String,
      default: undefined
    }
  },
  computed: {
    hasOffset() {
      return this.$slots.header || this.$slots.offset || this.title || this.text
    },
    styles() {
      if (!this.hasOffset) return null
      return {
        marginBottom: `${this.offset}px`,
        marginTop: `${this.offset * 2}px`
      }
    }
  }
}
</script>
 <style lang="scss">
.header-ti {
  width: 100% !important;
}
</style>